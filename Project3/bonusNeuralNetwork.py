...
# initialize
TRAINING_FILE = 'mnist_train_0_4'
TESTING_FILE = 'mnist_test_0_4'
...
class NeuralNetwork():
    ...
    def multilayer_perceptron(self, X: np.matrix, Y: np.matrix, alpha: float):
        ...
        weights_h = np.random.uniform(-1, 1, (X.shape[1], self.num_hidden_layer_nodes)) # [784 x 10]
        weights_o = np.random.uniform(-1, 1, (self.num_hidden_layer_nodes, self.num_output_nodes)) # [10 x 5]
        biases_h = np.random.uniform(-1, 1, (self.num_hidden_layer_nodes, 1)) # [10 x 1]
        biases_o = np.random.uniform(-1, 1, (self.num_output_nodes, 1)) # [5 x 1] 
        
        for i in range(len(X)):
            example = X[i][:, np.newaxis] # [784 x 1]
            # create class label vector which the row entry at class value level is 1, other entries are 0
            class_label = np.zeros((self.num_output_nodes, 1)) # [5 x 1]
            class_label[Y[i]] = 1
            ### forward pass 
            # generate hidden outputs
            # ([10 x 784] x [784 x 1] = [10 x 1]) + [10 x 1] = [10 x 1]
            hidden_nodes = np.dot(weights_h.transpose(), example) + biases_h
            activations_h = list(map(self.activation_function, hidden_nodes))
            # generate the full output
            # ([5 x 10] x [10 x 1] = [5 x 1]) + [5 x 1] = [5 x 1]
            activations_o = self.activation_function(
                        np.dot(weights_o.transpose(), activations_h) + biases_o)
                
            ### backpropagation
            # calculate deltas
            # ([5 x 1] - [5 x 1] = [5 x 1]) * [5 x 1] = [5 x 1]
            delta_o = (class_label - activations_o) * self.deriv_activation_function(class_label)
            # ([10 x 5] * [5 x 1] = [10 x 1]) * [10 x 1] = [10 x 1]
            delta_h = np.matmul(weights_o, delta_o) * self.deriv_activation_function(hidden_nodes)
        
            # propagate deltas backward
            # [784 x 1] x [1 x 10] = [784 x 10]
            ...
            # [10 x 1] * [1 X 5] = [10 x 5]
            weights_o += alpha * np.outer(hidden_nodes, np.transpose(delta_o))
    ...
    def predict(self, X: np.matrix):
        ...
        for i in range(len(X)):
            ...
            # classify example
            predictions.append(np.argmax(activations_o))
